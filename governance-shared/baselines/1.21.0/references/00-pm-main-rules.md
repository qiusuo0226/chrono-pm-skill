...
| 倒排计划编排 | 用户说"根据X月X日做倒排计划/围绕某截止日排任务" | 按 §9 WF-7 执行：澄清目标 → 查重 → 反向 WBS → 草案确认 → 写迭代 WP 表 + board Task |
| 待办创建/归属排布 | 用户说"给XX加个待办/任务""安排XX做YY"，或日报明日计划/纪要行动项/需求拆解/变更批准产生新任务 | 按 §9 WF-8 执行：归属判定（WP/独立/提醒三分）→ 正式任务强制落 board（带 WP Ref），一次性提醒才只落索引 |
...
### 工作包（WP）状态
`planned` → `in_progress` → `completed`（WP 进度由 board 中关联 Task 完成比例实时聚合派生，见 `03-task-board-rules.md` §8.0）
...
| 级联 | 18.5 | 待办**状态更新**时，执行 `03-task-board-rules.md` §8.1 状态级联检查；待办**创建**不在此步骤处理，统一路由 WF-8 归属判定（含日报/纪要/需求拆解/变更批准入口） | CHECK/SUGGEST |
...
触发：用户提交个人日报/项目日报。
路径摘要：读 project-context + board → 解析日报 → 加载需求上下文（Task 带 Requirement Ref 时）→ 汇总项目日报 → 检测资源变动 → 同步待办索引 → 生成快照/实际 → 归档检查。
...
倒排 = 迭代计划的一种编排方式，不另立体系：从锚点日期（截止日）反向推算 WP 时间分布，落入（新建或既有）迭代，再拆 Task 落 board。
...
| 查重 | iteration-register（既有迭代）、board（既有任务） | — |
| 反向 WBS | requirement-register、resource-register | — |
| 草案 | board（仅当存在落库 Task；初始草案时以 WBS 产出为准）、iteration-register（若存在，识别倒排 WP 范围） | 输出 WP 表 + Task 拆分建议 + 倒排元数据（目标/锚点/关键路径/缓冲）+ **倒排每日矩阵** + 风险提示，待 PM 确认。矩阵数据源：(a) 初始草案（PM 首次创建倒排）= WF-7 本次反向 WBS 产出的拟建 WP/Task 列表（非 board，因 Task 尚未落库）；(b) 后续刷新/查询 = 走 05号 §6.7 倒排每日矩阵路由从 board 实时派生；(c) 若 PM 口述了具体安排但 board 未更新 → 先执行 WF-8 落 board 再生成矩阵 |
| 落库 | — | iteration-register（WP 粗规划表 + 倒排元数据，SUGGEST）、board（Task + WP Ref + Due Date，SUGGEST） |
| 级联 | requirement-register、milestone-board、risk-register | [CHECK] 需求/里程碑关联存在；[SUGGEST] 工期紧张/资源冲突登记风险 |
| 验证 | — | **级联完成验证**（§8a.2）：确认所有级联动作全部执行或呈现 | MANDATORY |

### WF-8 待办创建与归属排布（所有任务创建入口的统一前置规则）

触发：**任何入口**产生新待办/任务——① 用户口述"给XX加个待办/任务"；② WF-2 日报明日计划提取（01号 §5.6b）；③ WF-3 纪要行动项提取（02号 §2）；④ 需求拆解（07号 §3.2）；⑤ 变更批准后新增/调整任务（08号 §6.1）。
待办不是独立数据实体，是一个输入入口：AI 必须先判定归属，落到唯一位置；正式任务只有 board 一个可写载体，禁止"只写索引不落 board"。

| 阶段 | 动作 | 强度 |
|------|------|------|
| 归属判定① | 读 iteration-register 总览表 → 按 Owner + 当前迭代时间窗口预筛候选 WP（最小读取） | MANDATORY |
...
| 落位 | 归属 WP → board Task（WP Ref + Due Date）；独立任务 → board Task（WP Ref: none）；一次性提醒 → todo 索引（唯一不落 board 路径，须输出判定理由） | 正式任务 MANDATORY |
| 继承 | 需求拆解/变更批准入口：需求已映射迭代 WP → 直接继承 WP Ref；Requirement Ref（需求溯源）与 WP Ref（执行归属）并存不冲突 | MANDATORY |
| 级联 | 写 board → [AUTO] 派生 todo 索引 → [AUTO] 汇总 WP 进度；以上为同一建议更新清单内的原子动作，不留半更新 | AUTO |
| 验证 | **级联完成验证**（§8a.2）：确认 board/索引/WP 级联全部执行或呈现 | MANDATORY |

> 兜底：任何新 Task 落位时（含直接看板建任务等边缘入口），若上下文可识别 WP 归属则补填 WP Ref。
...
entity-registry.md 不存在时，AI 应从 board/日报/需求文件中推断实体并在输出中标注"推断，未经台账校验"。

**优先级规则**：entity-registry 是 board / requirement-register 事实源的**派生投影**，不是独立事实源。
当 registry 与 board 出现实体/阶段差异时：
- 以事实源为准
- 反向提示"⚠️ entity-registry 需同步"
- 除非命中 `05-query-rules.md` §3(3)a 终态事件豁免（此时 registry 中的终态事件记录优先于 board 状态）
...
| 联调通过 / 集成验证通过 | 开发 + 集成完成 | 代码已完成并集成 | 相关任务 → review / done |
| 测试通过 | 测试阶段完成 | 质量已验证 | 相关任务 → done |
| 验收通过 | 交付完成 | 用户已确认 | 所有相关任务 → done |
| 上线完成 | 部署完成 | 生产环境已就绪 | 所有相关任务 → done |
...
1. **推导结论 → SUGGEST**：推导出"开发已完成"后，必须输出"建议同步任务板状态（T-XXX → review/done）"的 SUGGEST
...
3. **推导输出格式**（复用 §4 信息追溯规范）：
   ...
   - 推导结论：[具体结论]
   - 建议动作：[SUGGEST 同步任务板状态]
   ```
4. **级联触发**：推导产生的任务状态变更建议，触发 `03-task-board-rules.md` §8 级联传播规则

### 10.5 推导→任务集关联

推导出结论后，AI 必须定位受影响的任务集。识别优先级：

1. **entity-registry 关联任务列**：实体行中的"关联任务/需求"列直接列出 Task ID / Requirement Ref
2. **board 任务标题匹配**：按实体名 + 模块名在 board 任务标题中语义匹配
