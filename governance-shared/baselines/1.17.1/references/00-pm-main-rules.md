...
| 编号 | 规则 | 说明 |
|---|---|---|
| CQ-4 | 待确认事项必须编号 | 待确认/待讨论事项必须用 1、2、3 编号罗列，每条含简要描述和建议动作。不得仅以表格呈现而不编号——编号便于用户在对话中引用（如"确认第 2 项"） |
| CQ-5 | 实读禁缓存 | 查询回答必须基于本轮实际读取的文件内容。禁止凭 AI 记忆或上一轮对话缓存答复。若本轮未读取某文件，不得引用该文件内容作为结论依据。与 `05-query-rules.md` §6 最小读取原则互补：最小读取控制"读哪些"，CQ-5 确保"读的必须真读" |
...
| 级联 | 18.6 | 若本次涉及 Task Owner 委派 → 执行 `03-task-board-rules.md` §8 委派跟踪级联（为委派方生成跟进待办） | CHECK/SUGGEST |
