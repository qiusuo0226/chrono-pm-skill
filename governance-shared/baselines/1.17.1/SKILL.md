...
| 文件 | 核心内容 | 何时加载 |
|------|----------|----------|
| `00-pm-main-rules.md` | 角色定位、管理原则、行为边界、输出规范、§5.1a 沟通质量规则、意图检测 | 所有场景必须 |
| `01-daily-report-rules.md` | 日报生成、个人→项目→周报联动、资源变动检测、§6 集成审查模式、§7 主动提问 | 日报处理 |
| `02-meeting-rules.md` | 会议纪要生成、行动项提取、事实源同步、§6 级联传播规则（规则层）、decision-log 归档 | 会议处理 |
| `03-task-board-rules.md` | 任务看板字段、状态流转、计划变更/延期计数、超期判定归属、§8 级联传播规则（含状态变更需求一致性、委派跟踪） | 任务管理、计划变更/超期 |
| `04-risk-issue-rules.md` | 风险/问题识别、评估、升级机制、§9 级联传播规则、§9.1 关闭确认佐证 | 风险/问题管理 |
| `05-query-rules.md` | 查询路由、§2.0a 查询默认过滤、跨项目查询、聚合计数秒答、最小读取原则 | 项目状态查询 |
