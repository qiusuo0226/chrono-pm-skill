...
| 上线范围 / 批次 / 哪些（不）上线 / 十一窗口 | 「开办变更注销哪些 10 月 1 不上线」 | **先** `plans/_index.md`（business_modules/batch/time_window/scope_exclude），再最多打开 1～2 个命中 PLAN。**禁止先读** risk-register。结论必须含归属 + PLAN 号。缺 index：提示一次是否重建；未同意则读 PLAN 标题/§1 并标「锚点未填」，**禁止查询中全量扫描**（14 §2.1）。锚点为 — 不猜排除范围 |
