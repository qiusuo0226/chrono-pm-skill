...
| 项目日报查询 | "X月X日的项目日报？""上个月项目日报" | 内容查询（风险/问题/进度）默认先读 todos/{date}/*.md §2+§3。仅当用户明确要「项目日报文件」时读 reports/daily/project/YYYYMM/（YYYYMM 单级目录，v2.1.0 R-2；月度索引 index.md 见 01 号 §4.1）。目录不存在 = 未生成过，提示可按需生成，不报错。个人日报不在此路径。 |
...
**数据读取路径（按优先级）：** ①PM 当日待办文件 `todos/{date}/{PM姓名}.md`（PM 待办）→ ②当日绑定文件 `todos/{date}/_index.md`（全团队入口）→ ③本项目 `todos/{date}/{owner}.md` 待办文件（进度）→ ④`risks/risk-register.md`（open 风险）→ ⑤`issues/issue-register.md`（未关闭问题）→ ⑥PLAN 文件（WP 所属阶段）→ ⑦本项目 resource 相关文件（资源变动）→ ⑧本项目最近日报缓存 → ⑨本项目最近会议纪要。
