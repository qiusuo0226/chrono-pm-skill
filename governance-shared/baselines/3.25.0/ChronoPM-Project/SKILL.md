...
若工作区是联邦集工作区中的 `ai/projects/{项目名}/ai/`，本包仍按单项目读写该目录，禁止写兄弟项目，禁止写 `portfolio/`。允许被集层 P-HANDOFF-WRITE 调用（root=本成员）。
