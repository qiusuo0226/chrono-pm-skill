...
**位置**：项目级 `requirements/contract-register.md` 为拆解发生地（每项目一份）。拆解产物按 `requirements/{type}-source/` 统一英文目录留存（8 类：approval-source / bid-source / contract-source / credit-source / external-source / registration-source / **tender-source** / **security-assessment-source**）。文档簇 ID（CON-/BID-/INIT-/COMP-）为登记册逻辑标识与去重键，不再作为目录名。

- **单簇暂不平铺**：`{type}-source/` 下 ledger 记 `source_id`，簇 ID 写在登记册；seg 文件平铺。
- **多簇**再按 `{type}-source/{簇 ID}/` 平铺。
- **禁止第三套目录**（不得再使用 `contracts/CON-NNN` 等旧目录名）。
...
#### 8.9.5 ledger.md 字段规范（{type}-source 拆解台账，v3.0.0 P-25）

每个 `requirements/{type}-source/` 目录配一份 `ledger.md`，登记本次拆解的来源文件清单。**模板契约不得只靠兼容兜底**，字段如下：

| 字段 | 必填 | 说明 |
|---|---|---|
| source_id | 是 | 来源文件序号（如 SRC-001）；单簇时簇 ID 在登记册，本字段仅局部唯一 |
| file | 是 | seg/来源文件名（相对 {type}-source/ 路径） |
| source_fingerprint | 是 | 轻量指纹：优先内容哈希；不可得时用「文件大小+修改时间」（Portfolio 02 号兼容口径） |
| parsed_by | 是 | 拆解执行者（AI 会话标识或人员），追溯拆解质量责任 |
| parsed_at | 是 | 拆解日期 YYYY-MM-DD |

跨项目检索由 ChronoPM-Portfolio 遍历 + `source_fingerprint` 去重：指纹相同视为同一文档的跨项目副本，不重复拆解。存量无 ledger 的 {type}-source/ 目录，首次触碰时补建（SUGGEST → 待确认），不回填猜测值。
