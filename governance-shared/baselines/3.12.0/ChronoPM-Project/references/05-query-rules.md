...
| 查询场景 | 数据访问 | 输出 |
|---|---|---|
| 计划整体进度（PLAN-NNN） | 先闸 2（正常计划）：§3 与 WP 链尾/执行人/时间盒对账，不一致先修。废弃计划告知已废弃+`superseded_by`，不投影。读 `wps/_index.md` + `wps/WP-*.md` + PLAN §3；**计划进度只聚合 WP，禁止从 PLAN 正文待办行统计**；待办按 WP Ref 实时聚合（不回写 PLAN）。排期查询**只读 status=正常** 的计划 | WP 列表 + 各 WP 进度 + 计划总进度 |
| 某 WP 进度/状态（WP-NNN） | 先定位 `wps/WP-NNN.md`；**当前状态=§7 链尾**（无 §7 则降级报头并标明状态链未建）；待办按 WP Ref 过滤（实时聚合，不回写 WP §4） | 当前=链尾 + 变化线 + 进度。进度=Σ绑定待办进度÷绑定数；无绑定=0%；进度列「—」按 0% 计入 |
| 这计划谁没做完 / 还差谁 | 闸 2 后读正常计划 §3 | 链尾不是已完成/上线的 WP；执行人=`—` 的行 |
| WP 在哪些计划 | 读 WP `plan_ref`（正常）；问历史再扫废弃 §3 | 正常计划列表 |
| 哪些包没进任何正常计划 | 有 WP 文件但 plan_ref 为空或不含任何正常 PLAN | 未入包清单 |
