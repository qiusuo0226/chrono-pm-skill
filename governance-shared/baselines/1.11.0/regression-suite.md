...
| BP-002 | 对比 skill.json.version 与 Blueprint §1 版本 | 版本一致（均为 1.11.0） | positive |
...
| SC-1H | `python -c "from _version import SKILL_VERSION, WORKSPACE_SCHEMA_VERSION"`（sys.path 含 `scripts/`） | 输出 `SKILL_VERSION == 当前发布版本`（如 1.11.0）、`WORKSPACE_SCHEMA_VERSION == "0.6.0"`（单一版本源生效） | positive |
| SC-1I | `init_workspace.py --mode single` 在临时目录运行后读取 `.skill-version.json` | 断言 `skillVersion` 等于单一版本源（如 1.11.0），且与 `_version.SKILL_VERSION` 一致（init 链路端到端） | positive |
| SC-1J | 读取 `migrate_workspace.VERSION_CAPABILITIES` | 断言最大 `version` 字段 == 当前发布版本（如 1.11.0），且包含 1.9.0 条目（能力表补全） | regression |
