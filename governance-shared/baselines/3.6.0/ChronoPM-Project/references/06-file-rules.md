...
6. **团队信息指针化**：`project-brief.md` 中的团队信息不应复制 `resource-register.md` 完整团队列表，而应使用指针。brief 团队部分替换为：
   ```markdown
   ## 3. 团队成员
   → 见本项目 `resources/resource-register.md`（人员当前状态主源）
   → 见本项目 `resources/transfer-log.md`（人员流转历史）
   ```
   **迁移规则**：不自动删除 brief 已有团队列表，只新增主源指针并标记冗余待清理。下次 register 更新时提示用户"brief 团队列表已指针化，建议删除冗余信息"，确认后删除。

| 目录 | 是否允许创建AI文件 | 说明 |
|---|---|---|
| `ai/` 下本项目目录（todos/risks/issues/plans/requirements/decisions/resources/reports/meetings/context/outputs/logs） | ✅ 允许 | 本项目管理工作区 |
...
ai/ ├── todos/ ├── wps/ ├── risks/ ├── issues/ ├── plans/ ├── requirements/ ├── decisions/
   ├── resources/ ├── reports/ ├── meetings/ ├── context/ ├── outputs/ └── logs/
```
...
人员资源管理遵循「状态与历史分离」原则，事实源只在本项目 `resources/`：

| 文件 | 定位 | 内容 | 更新方式 |
|---|---|---|---|
| 本项目 `resources/resource-register.md` | 当前状态事实源 | 本项目人员当前状态快照（角色/状态/分配方式/B角等） | 覆盖更新（每次只保留最新状态） |
| 本项目 `resources/transfer-log.md` | 流转历史记录 | 本项目人员进出、调配、角色变更的完整流水 | 追加更新（只增不改不删） |
...
5. 跨项目共享人力索引不在本包维护；查询请使用 ChronoPM-Portfolio。

### 1.6 单项目资源管理（自 09 号 §8.1/§8.2 迁入）

路径一律本项目 `resources/`（及本项目 `logs/` 归档）。

**transfer-log 归档：**
- 拆分触发：>100 条记录 或 文件 >300 行
- 拆分策略：按年度归档到本项目 `logs/archive/YYYY-transfer-log.md`
- 归档后维护本项目 `logs/index.md` 索引
- 归档操作本身记录在 Change Log 中
- transfer-log 为只追加型文件，归档只移动历史条目，不删除任何内容

**resource-register 离场 90 天归档：**
- 状态为 `已离场`/`已调出` 且离场超过 90 天的人员 → 归档到本项目 `resources/resource-register-archive.md`
- 归档触发：在周报生成时检查
...
resources/resource-register.md
resources/transfer-log.md
```
...
**语义**：查找加速器。存在性以 `requirements/sources/*/meta.md` 为准；缺行补行。禁止再新建 `{type}-source/`。
