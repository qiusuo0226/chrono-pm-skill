...
| 版本范围 | 检查项 |
|---|---|
| 0.3.0+（仅旧工作区升级探测） | 本项目 `resources/resource-register.md` 和 `transfer-log.md` 是否存在（仅对 <2.0.0 升级来的工作区有意义；若仍见集层 `portfolio/resources/` 实体文件，提示按 3.0.0 迁移下沉，本包不写集层） |
...
| 3.6.0+ / schema 0.11.0 | `requirements/sources/` 与 `_index.md` 应存在。存量 `{type}-source/` 未迁完 → 截止条件：首次新拆解或首次 RI 范围判定前必须完成一次性迁移（清单→PM确认→先迁后验）。未完成 fallback 可读旧目录 |
...
## 10. archive 豁免规则（v2.1.0 新增）

`archive/` 目录（含 `archive/v1-legacy/` 及各实体归档分片）下的文件为**只读历史存档**，不参与完整性巡检（见 `14-self-check-rules.md`）：

1. 巡检字段缺失/格式不规范/状态异常等检查项均跳过 archive 目录下文件
2. AI 不得将 archive 下文件作为活跃数据源/兜底源（与 `06-file-rules.md` §12 禁用清单联动）
3. archive 下文件不可修改；发现需更正的历史内容应在活跃事实源中登记更正记录，不改写归档原文
