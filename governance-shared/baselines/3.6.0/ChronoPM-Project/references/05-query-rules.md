...
查询处理前（读取事实源之前），先检查本项目 `ai/pending-changes.md` 是否存在待确认项：

- `total_pending > 0` 时，在查询结果正文之前输出"**存在 N 项待确认变更**"提醒区块：编号 + 简述 + 建议动作（确认/驳回/查看）。禁止等 PM 主动问「还有什么没处理」（00 号 §8b.3）。
- 查询数据中与待确认变更直接相关的条目，在正文数据后标注 `(待确认)`。
- 若本轮即处理待确认项（PM 在确认/驳回），则不重复输出提醒区块。
- 简单查询不加载 `pending-changes.md` 全文（见 §6.3），仅执行确认/驳回/管理待确认项，或查询结果受待确认变更影响时加载；本会话已读过则仍须在开头提醒。
- 待办未绑定 WP 类 pending 的输出句式走 DF-019（00 号 §8b.4）。
...
|---|---|---|---|
| 风险 | `R-{YYYYMMDD}-{HHmmss}` | `R-NNN` / `R-YYYYMMDD-NNN` | 04 号 §2.2 |
| 问题 | `I-{YYYYMMDD}-{HHmmss}` | `I-NNN` / `I-YYYYMMDD-NNN` | 04 号 §4 |
...
| TD 编号反查 | "TD-CJJ-20260820-001 是谁的？""TD-SJJ-xxx" | resource-register「TD 编号治理」**双键**：先匹配现行缩写，未中再匹配历史别名；命中后读该人 todos。旧号不重编。跨项目归并请用 Portfolio（按中文名，不按缩写） |
| 需求查询 | "需求完成多少了？" | requirements/requirement-register.md |
| 源文档 / 拆解产物 | "这份合同拆了没？""SRC-001 摘要" | 先读 `requirements/sources/_index.md` → `_digest.md`；取证才读 atoms/facts。未迁完 fallback `{type}-source/` |
...
| 资源查询 | "团队有几个人？""张三现在在哪个项目？" | 本项目 resources/resource-register.md, transfer-log.md（跨项目查询请使用 ChronoPM-Portfolio） |
| 资源流转查询 | "张三什么时候调走的？""本周人员变动" | 本项目 transfer-log.md（按日期筛选） |
...
2. **默认只列未办结**：待处理 / 进行中 / 待评审 / 已阻塞。
3. **已办结默认不列**：已完成 / 已取消 / 已转出。
4. **未确认终态例外**：`Confirmed By=待确认` 的终态（含 DF-013 自动完成）默认仍可见——列未办结行或单列「待确认完成」分组，**禁止当已办结隐藏**。首行摘要可标注待确认数。
...
**数据读取路径（按优先级）：** ①PM 当日待办文件 `todos/{date}/{PM姓名}.md`（PM 待办）→ ②当日绑定文件 `todos/{date}/_index.md`（全团队入口）→ ③本项目 `todos/{date}/{owner}.md` 待办文件（进度）→ ④`risks/risk-register.md`（open 风险）→ ⑤`issues/issue-register.md`（未关闭问题）→ ⑥PLAN 文件（WP 所属阶段）→ ⑦本项目 resource 相关文件（资源变动）→ ⑧本项目最近项目日报存根（无则从 todos 汇聚）→ ⑨本项目最近会议纪要。
...
| 资源情况 | 本项目 resource 相关文件 | `transfer-log.md` | - |
...
| 查询意图 | 读取文件 | 输出格式 |
|---|---|---|
| 当前团队全貌 | 本项目 `resources/resource-register.md` | 本项目资源表格（姓名/角色/状态/分配方式） |
| 某人当前状态 | 本项目 `resource-register.md`（按姓名） | 单人信息卡（角色/所属项目/分配方式/B角/风险等级） |
| 某人流转历史 | 本项目 `transfer-log.md`（按姓名） | 时间线（流转日期/类型/来源项目/目标项目/原因） |
| 本周人员变动 | 本项目 `transfer-log.md`（按日期本周） | 流转记录表 + 影响分析 |
| 项目人手是否足够 | 本项目 `resource-register.md` + 00 号 §5c 容忍度 | 人力配置表 + 容忍度评估 + 风险提示 |

**资源查询回答原则：** ①当前状态以 `resource-register.md` 为准、流转历史以 `transfer-log.md` 为准；②某人状态为「借出/休假」须同时显示 B 角；③触发资源容忍度风险规则（见 00 号 §5c）须在末尾标注「资源风险触发」；④跨项目人员查询请使用 ChronoPM-Portfolio。
...
| 优先级 | 数据来源 | 用途 | 说明 |
|---|---|---|---|
| 1（主源） | 本项目 `resources/resource-register.md` | 人员当前状态 | 查询状态默认读本项目此文件 |
| 2（历史） | 本项目 `resources/transfer-log.md` | 流转历史追溯 | 仅查历史变动时读取；跨项目流转请使用 ChronoPM-Portfolio |
| 3（投影） | 本项目 `context/project-context.md` | 项目背景团队列表 | 是 register 投影，非独立事实源 |
| 4（候选） | `todos/{date}/` 待办文件（工作日志段） | 参与信号/候选证据 | 只能产生候选变更，不能自动认定为正式成员 |

**查询规则：** ①"本项目有哪些人/某人在哪/团队几人"默认只读 `resource-register.md`，不主动扫待办文件/transfer-log/project-context；②「所属项目」列含他项目名时，提示到对应项目对话 / Portfolio，本包不代查他项目文件；③字段为空/不存在（旧工作区）可从 `transfer-log.md` 推断但**标注"推断，未确认"**；④register 不存在则读 `project-context.md`，标注来源与可能不准确；⑤register 和 context 均不存在才扫待办文件（按 Owner 出现），标注"候选名单，未经确认"；⑥全无数据则输出"无法确定人员信息，建议初始化 resource-register.md"，不编造；⑦**禁止**：不得将待办文件出现过的人员自动认定为正式成员，不得未经确认自动修改 register。
...
| 人员状态 | 本项目 `resources/resource-register.md` | 不读 SKILL.md、不读 09 号（已退役） |
...
| 查询场景 | 数据访问 | 输出 |
|---|---|---|
| 计划整体进度（PLAN-NNN） | 先读 `wps/_index.md`（加速器）+ `wps/WP-*.md`（存在性以文件为准）+ PLAN §3 引用简表；抽取未完成 fallback 读计划内嵌表；待办按 WP Ref 聚合 | WP 列表 + 各 WP 进度 + 计划总进度 |
| 某 WP 进度（WP-NNN） | 先定位 `wps/WP-NNN.md`（索引可加速）；待办文件按 WP Ref 过滤（实时聚合，不回写 WP §4） | 待办列表 + 完成比例 |
...
| 倒排每日矩阵 | 读本项目 `ai/todos/{date}/` 待办文件 + PLAN 文件（若存在）；如需角色标注辅读本项目 resource 相关文件。跨项目矩阵由 ChronoPM-Portfolio 聚合 | 人员×日期矩阵（行=Owner，列=工作日，格=待办简述） |
