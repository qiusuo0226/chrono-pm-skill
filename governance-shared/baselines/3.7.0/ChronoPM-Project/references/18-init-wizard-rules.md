...
> **成本核算方式（R14）**：人工计量单位与默认能耗规则为必填，与合同总额同级。单位中立，规则/向导不得写死单一计量单位。成本单价选填（涉密可留空）。**两项缺失 → 向导不放行**，不得以「稍后补充」跳过；后续每轮对话持续提醒直至补录。能耗段单位取此处登记值。
...
**目标文件**：本项目 `resources/resource-register.md`（建议创建；人员亦可从待办/日报推导。跨项目共享人员提示用 ChronoPM-Portfolio，本包不写集层索引）
...
| Step 5 资源层 | `resources/resource-register.md` | 本项目资源清单 + 计划分配视图 |
