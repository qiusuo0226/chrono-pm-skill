...
│   ├── {子项目2}/
│   └── {子项目3}/
├── outputs/                             # 输出产物（Excel/导出文档，v2.1.0 起移入 ai/ 下）
└── logs/
...
ai/ ├── todos/ ├── risks/ ├── issues/ ├── plans/ ├── requirements/ ├── decisions/
   ├── reports/ ├── meetings/ ├── context/ ├── outputs/ └── logs/
```
...
>
> **v2.1.0 废弃路径标注**：`reports/daily/personal/`（任何子目录层级）已废弃，**禁止写入**（见 01 号 §1.2a 与本文件 §12 禁用清单）；周报路径统一为 `reports/weekly/YYYY/YYYY-Wxx.md`（R-1，以 01 号 §3.0 为准，不再使用 `YYYY-Wxx-[project]-周报.md` 旧命名）。
...
3. 跨层引用需全路径时显式带 `ai/` 前缀；01 号 §3.0 的 `ai/projects/...` 全路径写法保留（层级表需明示绝对位置），其余章节路径均为相对路径
