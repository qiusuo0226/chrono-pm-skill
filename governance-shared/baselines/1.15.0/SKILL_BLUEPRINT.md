...
| 当前版本 | 1.13.1（版本单一事实源为 `scripts/_version.py`） |
...
| —（CAP 扩展） | Requirement Intelligence (RI) | stable | L3 | `07` + `05` + `17` + `06` | requirements/atoms/(L1/L2/L3)+canonical | v1.15.0：跨源需求拆词/归并/范围判定/三级索引检索；归属现有 CAP 扩展（非独立 CAP），CR-20260813-001 |
...
| 09 | `09-portfolio-rules.md` | 项目集：汇总、跨项目风险、资源管理、§8 级联传播规则 |
...
> 对应 CR-20260813-001（v1.15.0）。覆盖跨源需求拆词、归并、范围判定与三级索引检索。

```
跨源输入（合同条款/招标文件/口述需求）
  → [07] 需求拆词 → 生成 ATOM（含 source_type/authority/raw_text/norm_text，写入 requirements/atoms/{category}）
  → [07] 登记 source_type 至 requirements/source-type-registry.md（未登记则触发提示，不静默归类）
  → [17] 词库同义词扩展 → 相同语义的 ATOM 建立关联
  → [07] 归并 → 归属同一 Canonical（requirements/canonical/，evidence 汇聚多来源）
  → [07][05] 范围判定 → 输出 scope + 证据链（合同/招标/口述来源比对）
  → [06] 维护三级索引（requirements/atoms/atom-index、{category}-index、{category}）
  → 查询经该三级索引检索（requirements/atoms/(L1/L2/L3)+canonical）
  → 合同出新版本 → 相关 ATOM 标记 stale、Canonical evidence_stale
```
