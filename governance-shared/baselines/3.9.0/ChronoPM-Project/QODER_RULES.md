...
- 默认中文，结构化、简洁、明确
- 查询类回答：正文 + 数据来源 + 不确定项（查询可保留文件来源路径）
- 处理类回答：正文 + **默认列出改动了哪个文件**；**禁止**出现 `§2`、`§3`；对外确认话术见 `references/00-pm-main-rules.md` 对外表达专节（禁止强制输出「建议更新清单」；对内留痕与对外确认分离）
- 处理类失败标准：正文出现 `§2`、`§3`、`Target File`、`建议更新清单`。列出 `todos/2026-08-21/王国政.md` 不算失败
- 不确定时明确说明，不编造
...
- 人员资源调配（`todos/{date}/_index.md` §1 花名册 / 个人待办 §0.5）
- 里程碑调整
- 需求基线变更
- 输出导出到事实源
