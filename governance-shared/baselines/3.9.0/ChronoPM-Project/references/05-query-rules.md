...
- 查过程用量、token、耗时、字段错误、漏条款、幻觉：先读 `ai/logs/ops/_index.md`（无则尚未发生过过程日志），再按 index 打开当日 `logs/ops/YYYY-MM-DD.md` / `-errors.md` / `-pN`。日志**不是**事实源，不能替代 todos/registers。
...
| 2（身份/流转） | 个人待办 §0（联系方式/负责模块）、§0.5（进出组） | 有待办者的细节与流转 | 无待办文件的人只在 §1，禁止建空待办 |
...
| 查询场景 | 数据访问 | 输出 |
|---|---|---|
| 计划整体进度（PLAN-NNN） | 先读 `wps/_index.md`（加速器）+ `wps/WP-*.md`（存在性以文件为准）+ PLAN §3 引用简表；**计划进度只聚合 WP，禁止从 PLAN 正文待办行统计**；待办按 WP Ref 实时聚合（不回写 PLAN） | WP 列表 + 各 WP 进度 + 计划总进度 |
