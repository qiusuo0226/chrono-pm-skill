...
```
用户提问 → 判断问题类型 → 按路由表加载对应提示词 → 前置检查 pm-decisions.md（见 §1a）与 inbox（见 §1b） → 读取相关事实源 → 按时间或ID定位过程记录 → 输出结论+信息来源+不确定项+冲突标注
```

### 1a 待裁定事项前置检查

查询处理前（读取事实源之前），先读本项目 `ai/pm-decisions.md` **八块开放行之和**（无该文件 = N=0，懒建，不报致命）：

- 开放行合计 N > 0 时，在查询结果正文之前用白话横幅：「**有 N 件事等你裁定**」——编号 + 简述 + 裁定选项。禁止等 PM 主动问「还有什么没处理」（00 号 §8b.3）。禁止用内部类型名 / `total_pending` / 块号当横幅。
- 查询数据中与待裁定事项直接相关的条目，在正文数据后标注 `(待确认)`。
- 若本轮即处理裁定（PM 在确认/驳回），则不重复输出提醒区块。
- 简单查询**可不加载** `pm-decisions.md` 全文（见 §6.3）；仅执行确认/驳回/管理待裁定项，或查询结果受待裁定变更影响时加载。**本会话已读过则仍须在开头提醒。**
...
| 某 WP 进度/状态 / 完整记录（WP-NNN） | 先定位 `wps/WP-*.md`；**当前状态=§7 链尾**；报 `effect`。废弃包告知+后继。待办按 WP Ref 实时聚合（不回写 WP §4）。未迁移文件若仍有「评审状态」列：原样展示已评审/未评审/—（脏数据待评审也原样），禁止翻译成十三阶段 | **摘要 + 原件链接**（见下款）。禁止无链接的纯总结 |
...
| 画图 / 责任链图 / 人员排布图 / Mermaid / 工作包总览（v3.17.0） | 读最新 `wps/_index.md` + 各 WP YAML `related_wps`。须加载 11 号 §17 | **默认**一张全包竖排总览（编号/包名/人员三行；边只认 related_wps；无则不连线）。对话输出 Mermaid；若已有 `wps/_wp-chart.md` 附链接。禁止 P-OUTPUT、禁止编造边。计划图/人员图仅点名才出 |

**工作包查询输出（v3.17.0）**：默认不把工作包 md 全文贴进对话框（计划查询「原样输出完整 PLAN」不变）。

1. **摘要**：当前状态（链尾）、生效、整包阶段要点、功能点要点、实时未办结待办/关联问题/上下游。完整记录类问法摘要更全，状态/进度类可更短。
2. **原件链接**：必须给出可点击的 `wps/WP-*.md` 路径，便于预览核对。
