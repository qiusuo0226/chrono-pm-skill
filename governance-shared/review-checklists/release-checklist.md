...
- [ ] 是否已运行 `python governance-shared/scripts/audit_release.py` 且退出码为 0？（未通过禁止发布）
...
- [ ] `governance/migrations/upgrade-to-{新版本}.md` 是否已创建？
  ...
  - 是否标注了对应的 CR 编号？
  - 若本版改查询形态/页规格/索引/关联：是否有「工作区存量」节（谁执行、失败不得盖戳）？
- [ ] 版本链是否连续？（每个版本都有对应升级文件，无断点；README 仅为当前指针，漏登以目录内 `upgrade-to-*.md` 为准）
- [ ] 分发包是否包含 `scripts/compile_source_digests.py`（本版及以后升级执行需要）？
...
- [ ] 是否已运行 `python governance-shared/scripts/audit_release.py` 且退出码为 0？（12 类断言统一守护）
