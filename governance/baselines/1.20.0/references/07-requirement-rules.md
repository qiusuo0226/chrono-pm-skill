...
5. **拆解出的 Task 落 board 前必须执行 `00-pm-main-rules.md` WF-8 归属判定填 WP Ref**：该需求已映射到迭代 WP（需求所属迭代的工作包）→ 直接继承 WP Ref；否则走 WF-8 三分判定。Requirement Ref（需求溯源）与 WP Ref（执行归属）并存不冲突。
...
**大文档渐进导入**：源文档不要求一次性全量导入，按章节/模块分批提取（每批约 2000-5000 字）；ATOM 的 `source_ref` 精确到章节号以支持断点续导；原始文档**不进入工作区**，仅在 ATOM 中存指针（source_doc + source_version + source_ref）。
...
RI 范围判定隐含"合同↔子项目 1:1"假设，但现实中为**多对多**（联合体合同、跨子项目合同、主合同+多补充协议、同一子项目被多份合同覆盖等）。本小节补齐该缺口：contract-register 作为合同作用域映射事实源，定义 ATOM/Canonical 的存储归属、带合同维度的 scope 判定与检索路由。

#### 8.9.1 contract-register.md 合同登记册（唯一映射事实源）

**位置**：项目集模式唯一登记册在 `portfolio/requirements/contract-register.md`（D4，子项目不复制）；单项目模式在 `requirements/contract-register.md`（D3）。所有主合同与补充协议统一登记。
...
#### 8.9.2 ATOM/Canonical 存储归属（D2/D3/D7）

| 合同 scope_level | ATOM 存在哪 | Canonical 存在哪 |
|---|---|---|
| `portfolio`（跨子项目） | `portfolio/requirements/atoms/` | `portfolio/requirements/canonical/` |
| `project`（单子项目或单项目） | `projects/{子项目}/requirements/atoms/`（单项目 `requirements/atoms/`） | `projects/{子项目}/requirements/canonical/`（单项目 `requirements/canonical/`） |
| `supplement`（补充协议） | **跟随父合同 scope_level**（D7） | **跟随父合同 scope_level**（D7） |

**Canonical storage_level 判定（D2）**：Canonical 是跨源归并产物，其归属由 evidence 的层级决定：
- evidence 全部来自同层级合同 → 归该层级；
- evidence 跨 `portfolio`/`project` 或多子项目 → 归 portfolio 级，`storage_level=portfolio`；
- 单项目 multi-合同（场景 D/H）不引入 portfolio 目录，Canonical 归 `requirements/canonical/`，用 `contract_refs` 区分合同。
...
```
Step 0  读 contract-register（空 → 触发补录引导，最小字段：ID/名称/scope_level/覆盖/status）
Step 1  解析合同指向
        ├─ 指定合同（CON-XXX/名称）→
        ...
        ├─ 未指定 → 列合同候选供选择；或"全部范围"→ 逐合同检索合并
        └─ 登记册空/无匹配 → 提示补录，不臆造（D5）
Step 2  目标 canonical 走 L1→L2→L3 三级索引（单次 200-400 行最小读取，05 号）
...
双视图链路字段完整性纳入 19 号信息完整性巡检（**存在性校验，非强制必填**）：已填 Requirement Ref 的 Task 校验其 REQ 存在且未 cancelled；confirmed 状态 REQ 的实现视图/原型链接覆盖率作巡检提示（Requirement Ref 本身为 03 号定义的可选字段，不得变相强制）。
