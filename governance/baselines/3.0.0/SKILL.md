...
**待办单一数据源**：执行状态 = `todos/{date}/{owner}.md`；PLAN = 唯一计划编排事实源；WP 进度 / 日周计划 / todo 索引均为派生。
...
| `plans/PLAN-*.md` `progress-plan.md` `budget.md` | 计划/进度/预算 |
...
### 5.2 日报
先 §1.0 录入归属判定 → 存档 §2 → 映射待办。疑似他项目：拆分+分流，禁代写。见 `01-daily-report-rules.md`。
### 5.3 查询
本项目事实源。跨项目用 Portfolio。待办清单输出见 05 号新节（默认未办结；未确认终态仍可见）。
### 5.4 其他
需求变更 08；结转 22（含 Step 0.5 共享人力提示）；历史计划 15。
