...
2b. **快照与日计划职责边界**：快照 = 冻结的历史记录（"那天计划是什么"，用于计划 vs 实际对比）；日计划 = board 中按 Due Date 过滤的 Task（可调整的规划层，带 WP Ref，见 `03-task-board-rules.md` §8.0）。快照不承担日计划职能，日计划也不写入快照；两者并行不悖，快照生成机制不变。
