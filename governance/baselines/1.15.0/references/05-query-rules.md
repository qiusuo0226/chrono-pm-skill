...
| 某需求在不在合同/招投标/立项范围内（范围判定） | `requirements/atoms/atom-index.md`(L1) → 目标 `{category}-index.md`(L2) | 命中 ATOM 全文(L3) → `requirements/canonical/canonical-index.md` | 全量扫描所有 ATOM/category 文件 |

### Quick Query 路由注：RI 三级索引加载

跨源范围判定查询（"XX 需求在不在 X 范围内"）必须按三级索引分级加载，对齐 §6.1 最小读取：

1. 加载 L1 `requirements/atoms/atom-index.md`（路由表，~6 行）→ 判断搜索哪些 category。
2. 加载目标 `requirements/atoms/{category}-index.md`（L2 类别倒排，含 `norm_text` 摘要覆盖索引）→ keyword + 语义扫读定位候选。
3. 仅加载命中的 ATOM 全文（L3，3-5 条）→ 看 raw_text + evidence。
4. 关联 `requirements/canonical/canonical-index.md` → scope_scope + 证据链。
