...
def parse_wp(path: Path) -> dict:
    ...
    if "supersedes" in fm:
        aliases += _listish(fm["supersedes"])
    return {
        "id": wp_id,
        "name": name,
        "status": status,
...
def parse_glossary(path: Path) -> tuple[dict, list]:
    ...
    for row in _table_rows(s1):
        ...
        if status != "confirmed":
            continue
        alias[orig] = {"id": gid, "type": "term", "canonical": canon}
    s2 = _section(text, "2.")
...
def build_entities(wps: list[dict], tds: list[dict], risks: list[dict], people: list[dict], people_abbr: dict, gloss_alias: dict, corrections: list, facts_fp: str, as_of: str) -> dict:
    ...
    for w in wps:
        ...
        rec = {
            ...
            "path": w["path"],
            "aliases": w["aliases"],
            "supersedes": w["upstream"] and [] or [],
        }
        if live and w["status"] != "已完成":
            entities.append(rec)
        for a in w["aliases"]:
            alias_index[a] = {"id": w["id"], "type": "wp"}
        if not live:
            alias_index[w["id"]] = {"id": w["id"], "type": "wp"}
            alias_index[w["name"]] = {"id": w["id"], "type": "wp"}
    for e in tds + risks + people:
        entities.append(e)
        for a in e.get("aliases", []):
            alias_index[a] = {"id": e["id"], "type": e["type"]}
        alias_index[e["id"]] = {"id": e["id"], "type": e["type"]}
    for abbr, name in people_abbr.items():
        alias_index[abbr] = {"id": name, "type": "person"}
    return {
        "as_of": as_of,
