...
| 当前版本 | 3.24.0（ChronoPM-Project + ChronoPM-Portfolio 共用版本线；版本单一事实源为 `scripts/_version.py`） |
...
| —（CAP 扩展） | 点名分工 / 挂包先验 / 完整表 / 弱结构投喂 | stable | L3 | `00` 8c/WF-8 + `01` §1.5 + `05` §6.7–6.9 + `10` L2/L3；Portfolio `01` §2.1 + `02` V-14 | wp-template §3c/§4c；personal §0.7；ingest-map；daily-dispatch | v3.20.0：§3c 非进度 SSOT；C(P) 挂包；进度默认完整表；集层不代写；乙案落盘。v3.24.0：集层材料投喂默认先落 ingest+日志，不问要不要落库 |
