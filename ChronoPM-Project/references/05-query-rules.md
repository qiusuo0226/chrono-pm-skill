...
| 画图 / 责任链图 / 排布图 / Mermaid | `wps/_index.md`（+ todos `_index` / PLAN §3，见 11 号 §17） | — | 写成文件；走 P-OUTPUT；编造 WP 边 |
...
| 画图 / 责任链图 / 人员排布图 / Mermaid（v3.16.0） | 读最新 `wps/_index.md`；责任链图再读上游/下游列或 WP YAML `related_wps`（空则跳过该子图）；人员排布再读当日 `todos/{date}/_index.md` §1+§3。须加载 11 号 §17 | **仅对话**输出 Mermaid 代码块。禁止落盘、禁止 P-OUTPUT、禁止登记 outputs/index |
