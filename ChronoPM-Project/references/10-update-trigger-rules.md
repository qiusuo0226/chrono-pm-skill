...
|---|---|
| 记录一下 / 记一下 | 通用入库指令 |
| 整理记录 / 帮我整理 / 补全 / 回填 / 写进去 | 投喂入库（含工时表/能耗）；同一句又像分析又像记录 → 按入库，禁止停在分析。**整理成对外文件/导出表** → 走 11 号生成物（`ai/outputs/`），禁止写到项目根。一句两意 ASK 一次 |
| 更新一下 / 同步一下 | 更新事实源 |
| 登记一下 | 新增记录 |
| 归档一下 / 入库 | 文件归档 |
| 写进去 / 写到管理库 | 写入指定文件 |
| 放到项目里 / 纳入项目管理 | 纳入管理范围 |
| 拆文件 / 拆文档 / 拆解这份 / 把…拆进 / 入库源文档 | 源文档拆解入库。**不询问**「是否按 ChronoPM 解析」。必须走 source-split，产物 `ai/requirements/sources/{编号}/`。禁止用 11 号 HTML 或其他 Skill 替代 |
...
→ 触发：写/读 `wps/WP-*.md` + `wps/_index.md`；创建走 WF-8 溯源（00 号 §8b.5）。**状态变更须写 §7**。计划文件只改 §3 引用简表（投影列禁止当独立事实改）。
...
→ **不是**本信号：拆解需求（无源文件，走 07 §3）；拆工作包；拆待办；合同拆分为两份。
...
→ **不是**本信号：完整性巡检缺字段（19）；等确认（pm-decisions）；拆文件（源文档信号）；换 Portfolio 能做的跨项目查询；废弃工作包（P-WP-RETIRE）；**画图/责任链图/排布图/Mermaid**（查询，走 05+11 §17，禁止当更新意图、禁止落盘）。
