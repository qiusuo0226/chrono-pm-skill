...
**最小读取集**：V-1～V-14 一切聚合只读索引/摘要行（project-index、status 摘要、登记册表格行、词库表格行、PLAN 头与 §3 行），不读全文。全文仅集经理点名某项目细节时才读。各成员若有 `.state.json`，结论附 as-of（`facts_fingerprint` 短码）。禁止 `portfolio/cache/` 当查询数据源。
