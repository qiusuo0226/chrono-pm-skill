...
材料投喂默认识别并落 `portfolio/reports/ingest/{batch}/`（原件+抽出行）与集层日志，禁止停在摘要、禁止问要不要落库。细则 01 §2.1。
...
3. 集经理意图变更成员项目实体 → 只走内部 V-9（目标项目 + 目标文件 + 建议内容），不代写。对外白话确认，不对用户说「建议更新清单」。集层说「我是张三」**禁止**写各子项目 `pm-profile.md`；提示到对应项目 ChronoPM-Project 对话声明当前操作人。
...
| V-14 | 混报拆分 + 弱结构投喂 | 「这份日报分到各项目」/ 无表头进度表 / 跨项目清单 | project-index、花名册、进行中 index；点名才开 C(P)；**禁翻历史日待办全文与旧分发稿**；+1 只读锚点日 | 只写 `portfolio/reports/` 分发稿 + `reports/ingest/` 原件；**永不写成员项目** |
...
| 集周报 | 01 + 04 |
| 意图变更 / 内部 V-9 | 01 |
| 材料投喂（排期/日报/风险/合同/清单粘贴） | 01（先 §2.1） |
...
| 文件 | 何时加载 |
|------|----------|
| `01-readonly-boundary-rules.md` | 写禁、内部 V-9、落盘规则、投喂统一入口 §2.1 |
