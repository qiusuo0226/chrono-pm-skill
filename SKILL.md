...
description: Markdown 驱动的 AI 项目管理技能。当用户需要管理软件研发项目（尤其 To G / To B 政企数字化转型领域）时使用，覆盖需求管理、任务跟踪、进度管控、风险与问题、里程碑、成本与P&L、日报周报、会议纪要、决策记录、复盘、项目集统筹、人员资源协调与流转、初始化向导、迭代管理、信息完整性巡检等全链路。触发场景包括：初始化项目工作区、初始化项目向导、迭代规划与登记、处理个人/项目日报、生成周报、整理会议纪要、需求评审与变更、风险识别与评估、任务看板更新、项目状态查询、里程碑评审、项目复盘、项目集汇总周报、跨项目资源调配、信息完整性巡检与补全提醒。当用户提到"项目管理""日报""周报""风险登记册""任务看板""需求追踪""里程碑""ChronoPM""项目集""资源分配""人员流转""记录一下""更新一下""整理到项目里""归档""入库""评审""需求评审""设计评审""技术评审""验收""上线方案""会议纪要""会议记录""测试报告""客户反馈""接口文档"等关键词时触发。当用户提到"初始化项目""录入项目信息""设置项目基线""补全项目基础信息"时触发初始化向导。当用户提到"迭代""迭代规划""迭代登记"等关键词时触发迭代管理场景。当用户提到"完整性巡检""缺失信息""补全提醒""信息不完整"时触发完整性巡检场景。当用户上传或粘贴文件（评审材料、会议纪要、日报、需求文档等）并要求处理时也触发。当用户要求"生成周报""生成报告""导出Excel""导出文件""做一份汇报材料""整理成文件""出个表"等生成物请求时也触发。当用户提到"术语""词库""缩写""纠正""domain-glossary"等关键词时触发词库管理场景。当用户提到"我的偏好""习惯设置""PM Profile""偏好学习""以后按这种格式"等关键词时触发 PM 偏好管理场景。支持 PM 偏好学习与输出适配，AI 在交互中被动观察用户习惯，经确认后影响输出格式、交互风格、信息侧重。适用于以 Markdown 文件为事实源、以 AI 为辅助、以人工确认为最终控制点的项目管理模式。
---
...
适用于独立项目。所有管理文档放在项目根目录下的 `ai/` 中。

### 2.2 项目集模式（portfolio）

适用于一个项目集经理统筹多个子项目的场景。所有管理文档集中在项目集根目录下的 `ai/` 中，按 `portfolio/`（项目集级）和 `projects/{子项目名}/`（子项目级）分层管理。

**项目集模式核心原则**：
- 所有 AI 管理文件只存在于根目录 `ai/` 下，不得侵入业务子项目目录
- `prompts/` 和 `templates/` 只保留一份，所有子项目共用
- 项目集级管理跨项目事项（汇总周报、整体P&L、跨项目风险、人员资源协调）
- 子项目级管理各自项目内事项（需求、任务、日报、风险）
- 向上汇总，向下不下沉：信息从子项目流向项目集，项目集决策通知子项目执行但不直接改子项目事实源
...
│   ├── projects/    # 子项目级管理（每子项目含 requirements/plans/tasks/meetings/reports 等）
│   └── logs/        # 操作日志
...
与项目集模式中单个子项目的目录结构相同，直接放在 `ai/` 下（无 `portfolio/` 和 `projects/` 分层）。
...
检测到新工作区或用户说"初始化项目"→ 读取 `project-brief.md` → 启动六步向导（合同层→项目层→迭代层→需求层→资源层→里程碑层）→ 生成确认摘要 → 写入 → 状态改"已确认"。详细规则见 `references/18-init-wizard-rules.md`。
...
发现人员变动 → 识别类型 → 登记 transfer-log → 更新 resource-register → 检查资源风险 → 联动子项目任务/成本 → 输出建议清单。详见 `references/09-portfolio-rules.md`。
...
| 用户说"记录一下/更新一下" | 00 + 06 + 10 | 按事项类型加载 |
...
| 日报处理（含个人进度联动） | 00 + 01 + 06 | 03、04、07、09、10 |
| 生成周报/报告 | 00 + 05 + 06 + 10 + 11 | 01、09 |
| 导出 Excel/Word/PDF | 00 + 06 + 11 | 相关业务规则 |
| 生成项目集周报 | 00 + 05 + 09 + 10 + 11 | 01、04 |
| 多轮修改生成物 | 00 + 06 + 11 | 10 |
| 归档生成物到 ai/ | 00 + 06 + 10 + 11 | 对应业务规则 |
| 生成 Excel 文件 | 00 + 06 + 11 + 12 | xlsx 技能 + 对应业务规则 |
| 导出需求跟踪矩阵 | 00 + 06 + 11 + 12 + 07 | xlsx 技能 |
| 导出风险/问题登记册 | 00 + 06 + 11 + 12 + 04 | xlsx 技能 |
| 导出成本测算表 | 00 + 06 + 11 + 12 | xlsx 技能 |
| 导出计划表/任务看板 | 00 + 06 + 11 + 12 + 03 | xlsx 技能 |
...
| 文件 | 核心内容 | 何时加载 |
|------|----------|----------|
| `00-pm-main-rules.md` | 角色定位、管理原则、行为边界、输出规范 | 所有场景必须 |
| `01-daily-report-rules.md` | 日报生成、个人→项目→周报联动、资源变动检测 | 日报处理 |
| `02-meeting-rules.md` | 会议纪要生成、行动项提取、事实源同步 | 会议处理 |
| `03-task-board-rules.md` | 任务看板字段、状态流转、关联规则 | 任务管理 |
| `04-risk-issue-rules.md` | 风险/问题识别、评估、升级机制 | 风险/问题管理 |
| `05-query-rules.md` | 查询路由、跨项目查询、资源查询、回答规范 | 项目状态查询 |
| `06-file-rules.md` | 文件命名、创建、更新、瘦身、业务目录不侵入 | 文件操作 |
| `07-requirement-rules.md` | 需求分类、拆解、评审、追踪矩阵 | 需求管理 |
| `08-change-control-rules.md` | 变更流程、影响分析、审批、分级 | 变更管理 |
| `09-portfolio-rules.md` | 项目集统筹、汇总周报、跨项目风险、人员资源管理 | 项目集管理 |
| `10-update-trigger-rules.md` | 更新意图识别、文件类型识别、语义信号触发、更新权限分级、目标文件路由 | 用户输入处理、文件解析入库、更新触发 |
| `11-output-artifact-rules.md` | 输出物目录规则、批次目录、草稿/确认/导出流程、文件格式询问、生成物索引、归档到ai/规则 | 生成周报/报告、导出文件、归档生成物 |
| `12-excel-generation-rules.md` | 8种文档Excel生成规范（sheet结构/列头/数据验证/公式/条件格式/冻结窗格）、成本测算询问式细化、生成后recalc+audit验证 | 生成Excel文件、导出登记册/计划表/周报/成本测算表 |
| `13-continuity-rules.md` | 项目阶段衔接（历史导入/结转/映射/冲突检测/不可覆盖）、continuity/目录、5种导入模式、内容路由表 | 导入历史项目、阶段衔接、结转遗留事项 |
| `14-self-check-rules.md` | 索引预建规则、索引一致性检查、文档处理自查清单（日报D1-D10/会议M1-M7/评审R1-R6）、风险问题多源交叉校验、遗漏补救、索引过期检测 | 文档处理后自查、风险问题追溯校验 |
| `15-snapshot-rules.md` | 计划快照（冻结）、实际执行摘要、历史快照索引、热/冷数据分离、计划vs实际对比、Todo ID稳定规则 | 历史计划查询、计划偏差对比、快照生成 |
| `16-skill-governance-rules.md` | 变更工单、影响分析、回归测试、核心契约保护、最小补丁、回滚规则、基线管理 | Skill 自身变更治理 |
| `17-domain-glossary-rules.md` | 领域术语词库、术语归一化、纠错、自动学习(pending)、确认式学习、索引、去重、扩容 | 术语/词库管理、日报/评审/查询/更新触发前置处理 |
| `18-init-wizard-rules.md` | 初始化向导六步流程、触发条件、跳过机制、进度记忆、确认写入规则、文件上传解析 | 项目初始化、新工作区首次使用、用户说"初始化项目" |
| `19-info-completeness-rules.md` | 日常信息完整性巡检、7层检查维度、P0-P3分级提醒、静默策略、巡检报告 | 生成报告、分析风险、推导计划、查询时完整性检查、用户说"完整性巡检" |
| `20-workspace-version-rules.md` | 工作区版本检查、健康检查、兼容模式、迁移模式、升级提醒、触发词 | 进入工作区首要动作、工作区健康检查、版本不匹配时 |
| `21-pm-profile-rules.md` | PM 偏好档案、用户习惯学习、5 类偏好分类、pending→confirmed 状态机、确认交互、冲突处理 | 所有场景自动加载（与 00 同级）、PM 偏好管理、用户说"我的偏好" |

### 版本控制文件

| 文件 | 用途 |
|------|------|
| `VERSION` | Skill 包版本号（当前 1.9.0）|
